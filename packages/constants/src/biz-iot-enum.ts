import { isEmpty } from '@vben-core/shared/utils';
...
/**
 * 设备状态汇总；label / value 给业务用，tagType 给 UI 层做颜色映射（不同框架可能各取所需）
 */
...
/** 产品设备类型枚举（与 DeviceTypeEnum 同义，保留旧名兼容旧代码） */
export const IoTProductDeviceTypeEnum = DeviceTypeEnum;
...
/** 兼容旧调用：保留函数形态 */
export const getActionTypeOptions = () => actionTypeOptions;
...
/** 兼容旧调用：保留函数形态 */
export const getConditionTypeOptions = () => conditionTypeOptions;
...
export const IotRuleSceneTriggerTimeOperatorEnum = {
  ...
  BEFORE_TODAY: { name: '在今日之前', value: 'before_today' },
  AFTER_TODAY: { name: '在今日之后', value: 'after_today' },
