...
async function onBtnClick(_value) {
  loading.value = true;
  try {
    const params = {
      assistantId: props.assistant.id,
      aiPrompt: aiPrompt.value,
      isTargetFilter: isTargetFilter.value,
    };
...
watch(
  ...
  (val: any) => {
    ...
    isTargetFilter.value = val.isTargetFilter;
    aiPrompt.value = val.aiPrompt || filterPrompt.value;
  },
  { immediate: true },
);
const list = ref([
      ...
      <div class="mb-6">
        ...
        <div class="flex gap-4">
          <Textarea
            ...
            placeholder="输入AI提示词..."
          />
          <Button size="large" type="primary" @click="onBtnClick">
            试试效果
          </Button>
        </div>
      </div>
