...
const URL_STICKY_REGEX =
  /(https?:\/\/[^\s一-龥@<>"']+|www\.[^\s一-龥@<>"']+)/iy;
...
export function parseTextSegments(
  ...
  const sortedMentions =
    mentions.length > 1
      ? mentions.toSorted((a, b) => b.name.length - a.name.length)
      : mentions;
  ...
  while (i < text.length) {
    if (text[i] === '@' && sortedMentions.length > 0) {
      const matched = sortedMentions.find(
        (m) => m.name && text.startsWith(m.name, i + 1),
      );
      if (matched) {
    ...
    if (head === 'h' || head === 'H' || head === 'w' || head === 'W') {
      ...
      if (linkMatch) {
        ...
        if (urlText.length >= URL_MIN_LENGTH) {
          flush();
          const href = /^https?:\/\//i.test(urlText)
            ? urlText
            : `https://${urlText}`;
...
export function toUserCardTarget(
...
export function toGroupCardTarget(
  group: GroupLite | null | undefined,
): CardTarget | null {
  if (!group?.id) {
    return null;
  }
 ...
 */
export function getCardLabelInfo(
  card: null | undefined | { targetType?: number },
): {
  icon: string;
  label: string;
} {
...
function buildSenderSnapshotMap(
  ...
  let groupMembers: Map<number, { avatar?: string; nickname: string }> | null =
    null;
...
export function buildMergeTitle(conversation: Conversation): string {
  ...
  const myName = useUserStore().userInfo?.nickname || '我';
 ...
 */
export function extractAddableFace(
  message: Message,
): AddableFacePayload | null {
  if (message.type === ImContentType.FACE) {
    ...
    return {
      url: face.url,
      width: face.width || 200,
      height: face.height || 200,
      name: face.name,
    };
  }
...
export const serializeMessage = <T>(payload: T): string =>
...
export const withQuotePayload = <T extends Quotable>(
  payload: T,
  quote?: QuoteMessage,
): T => {
  if (!quote) {
  ...
  try {
    ...
    if (playPromise && typeof playPromise.catch === 'function') {
      playPromise.catch((error) =>
        console.debug('[IM] playAudioTip 失败', error),
      );
    }
  } catch (error) {
    console.debug('[IM] playAudioTip 失败', error);
 ...
 */
export function getFileIconInfo(filename: string | undefined): {
  color: string;
  icon: string;
} {
...
export function resolveGroupNotificationSegments(
  ...
  const memberSegments = joinMentionSegments(
  ...
  switch (message.type) {
    ...
    case ImContentType.GROUP_BANNED: {
      return [
        operatorSegment,
        tipText(payload.banned ? ' 封禁了该群' : ' 解封了该群'),
      ];
    }
    ...
    case ImContentType.GROUP_MEMBER_NICKNAME_UPDATE: {
      return [
        operatorSegment,
        tipText(` 修改群昵称为 "${payload.displayUserName ?? ''}"`),
      ];
    }
    ...
    case ImContentType.GROUP_NAME_UPDATE: {
      return [
        operatorSegment,
        tipText(` 将群名修改为 "${payload.newName ?? ''}"`),
      ];
    }
...
export function parseRtcCallPayload(
  ...
  return content
    ? parseMessage<RtcCallEndPayload & RtcCallStartPayload>(content)
    : null;
}
...
function resolveRtcInviterLabel(payload: RtcCallStartPayload): string {
  return (
    payload.inviterNickname?.trim() || `用户 ${payload.inviterUserId ?? ''}`
  );
}
...
export function resolveRtcCallPrivateBubbleText(
  payload: null | RtcCallEndPayload,
): string {
  if (!payload) {
    return '通话已结束';
  }
  ...
  switch (payload.endReason) {
    ...
    case ImRtcCallEndReason.ERROR: {
      return hasDuration
        ? `通话中断 ${formatCallDuration(duration)}`
        : '通话中断';
    }
    case ImRtcCallEndReason.HANGUP: {
      return hasDuration
        ? `通话时长 ${formatCallDuration(duration)}`
        : '通话中断';
    }
