...
const dialogRef = ref<InstanceType<typeof UserSelectDialog>>(); // 用户选择弹窗
const hovering = ref(false); // 鼠标是否悬停
const selectedItems = ref<SystemUserApi.User[]>([]); // 已选用户列表
...
const showClear = computed(() => {
  return props.allowClear && !props.disabled && hovering.value && hasValue();
});
