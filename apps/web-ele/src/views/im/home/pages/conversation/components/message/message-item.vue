...
import {
  pinGroupMessage as apiPinGroupMessage,
  cancelMuteMember,
} from '#/api/im/group';
...
import {
  mediaTypeHandlers,
  useMediaUploader,
} from '../../../../composables/useMediaUploader';
...
const shouldShowTimeTip = computed(() => {
  ...
  return (
    props.message.sendTime - props.prevMessage.sendTime >
    MESSAGE_TIME_TIP_GAP_MS
  );
});
...
const isFriendChatTipMessage = computed(() =>
  isFriendChatTip(props.message.type),
);

/** 好友事件 segments */
const friendChatTipSegments = computed(() =>
  resolveFriendNotificationSegments(props.message),
);

/** 是否群广播事件（GROUP_CREATE..GROUP_BANNED 段位，排除 GROUP_MEMBER_SETTING_UPDATE 个人信号） */
const isGroupNotificationMessage = computed(() =>
  isGroupNotification(props.message.type),
);
...
const rtcCallEndPrivatePayload = computed(() => {
  ...
  return payload?.conversationType === ImConversationType.PRIVATE
    ? payload
    : null;
});

/** 是否私聊通话气泡 */
const isRtcCallPrivateBubbleMessage = computed(
  () => rtcCallEndPrivatePayload.value !== null,
);
...
const rtcCallTipSegments = computed(() =>
  resolveRtcCallTipSegments(props.message),
);
...
const showSendingLoading = computed(
  () =>
    props.message.status === ImMessageStatus.SENDING &&
    (!isUploading.value || isVoice.value),
);
...
const senderAvatar = computed(() => {
  ...
  if (conversation.type === ImConversationType.GROUP) {
    ...
    return (
      group?.members?.find((member) => member.userId === props.message.senderId)
        ?.avatar || ''
    );
  }
...
async function handleContextMenu(e: MouseEvent) {
  ...
  if (currentGroup.value && !props.message.selfSend && canManageSender.value) {
    ...
    const isMuted =
      senderMember?.muteEndTime &&
      new Date(senderMember.muteEndTime) > new Date();
    if (isMuted) {
      items.push({
...
function handleReply() {
  ...
  conversationStore.setConversationReplyDraft(
    conversation,
    buildQuoteFromMessage(props.message),
  );
}
...
async function handleKick() {
  ...
  try {
    ...
    await removeGroupMember({
      groupId: group.id,
      memberUserIds: [props.message.senderId],
    });
...
<template>
  ...
  <div
    v-else-if="isRtcCallPrivateBubbleMessage"
    ...
    >
      ...
      >
        <Icon
          ...
          :size="16"
          class="rotate-[135deg] flex-shrink-0"
        />
  ...
  >
    ...
    <div
      class="flex flex-1 min-w-0 gap-2 items-start"
      :class="{
        'flex-row-reverse': message.selfSend,
        'justify-center': isMaterial,
      }"
    >
